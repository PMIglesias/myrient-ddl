...
Write-Header "Instalador Myrient DDL"

# Configurar ruta de instalacion
$INSTALL_PATH = Join-Path $env:USERPROFILE "Myrient DDL"
...
$ZipFile = Join-Path $TEMP_PATH "myrient-ddl-complete.zip"
...
# Buscar myrient.db en la raiz de instalacion
$DbPath = Join-Path $INSTALL_PATH "myrient.db"

if (Test-Path $DbPath) {
    try {
        Write-Host "   Archivo myrient.db encontrado en raiz" -ForegroundColor Green
    ...
    $DbFile = Get-ChildItem -Path $INSTALL_PATH -Filter "myrient.db" -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
    if ($DbFile) {
...
try {
    ...
    $ShortcutPath = Join-Path $DesktopPath "Myrient DDL.lnk"
    
    $Shortcut = $WshShell.CreateShortcut($ShortcutPath)
    $Shortcut.TargetPath = $MainExe.FullName
    $Shortcut.WorkingDirectory = $INSTALL_PATH
    $Shortcut.Description = "Myrient DDL - Descargador de ROMs"
    $Shortcut.Save()
