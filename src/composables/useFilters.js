...
const TAG_PATTERNS = {
    regions: ['USA', 'Europe', 'Japan', 'World', 'Asia', 'Australia', 'Brazil', 'Canada', 'China', 'France', 'Germany', 'Italy', 'Korea', 'Netherlands', 'Spain', 'Sweden', 'UK'],
    languages: ['En', 'Es', 'Fr', 'De', 'It', 'Ja', 'Ko', 'Pt', 'Zh', 'Nl', 'Sv', 'No', 'Da', 'Fi', 'Pl', 'Ru'],
...
export function useFilters() {
    ...
    const applyFilters = (items) => {
        ...
        return items.filter(item => {
            ...
            if (advancedFilters.value.includeText.length > 0) {
                const includeMatch = advancedFilters.value.includeText.some(
                    text => title.includes(text.toLowerCase())
                );
            ...
            if (advancedFilters.value.excludeText.length > 0) {
                const excludeMatch = advancedFilters.value.excludeText.some(
                    text => title.includes(text.toLowerCase())
                );
            ...
            if (includeTagsActive) {
                const hasRequiredTag = tags.some(tag => {
                    return advancedFilters.value.includeTags.regions.includes(tag) ||
            ...
            const hasExcludedTag = tags.some(tag => {
                return advancedFilters.value.excludeTags.regions.includes(tag) ||
    ...
    const selectAllTags = (category, type, availableTags) => {
        const target = type === 'include' 
            ? advancedFilters.value.includeTags 
            : advancedFilters.value.excludeTags;
        
        const opposite = type === 'include'
            ? advancedFilters.value.excludeTags
            : advancedFilters.value.includeTags;
