/**
 * @fileoverview Módulo de gestión de base de datos SQLite para el catálogo de Myrient
 * @module database
 *
 ...
 * @property {boolean} success - Si la búsqueda fue exitosa
 * @property {Array<Object>} data - Array de resultados encontrados
 * @property {number} [total] - Total de resultados (para paginación)
 * @property {string} [error] - Mensaje de error si falló
 */

/**
 * @typedef {Object} NodeInfo
 * @property {number} id - ID único del nodo
 * @property {string} title - Título del nodo
 * @property {number} [parent_id] - ID del nodo padre (null si es raíz)
 * @property {string} [type] - Tipo de nodo: 'file' | 'folder'
 * @property {number} [size] - Tamaño en bytes (solo para archivos)
 * @property {string} [url] - URL de descarga (solo para archivos)
 */

/**
 * @typedef {Object} FTSTableInfo
 * @property {string} name - Nombre de la tabla FTS
 * @property {string} type - Tipo de FTS: 'fts5' | 'fts4'
 * @property {Array<string>} columns - Columnas indexadas para búsqueda
 */
...
/**
 * Servicio de base de datos SQLite para el catálogo de Myrient
 *
 * Gestiona la conexión a la base de datos, realiza búsquedas optimizadas con FTS,
 * y proporciona acceso a la información del catálogo. La base de datos se abre
 * en modo solo lectura para prevenir modificaciones accidentales.
 *
 ...
 * if (initialized) {
 *   // Buscar archivos
 *   const results = database.search('archivo', { limit: 50 });
 *   console.log(`Encontrados ${results.total} resultados`);
 * }
 */
class DatabaseService {
  /**
   * Crea una nueva instancia de DatabaseService
   *
   * @constructor
   */
  constructor() {
   ...
   *
   * Si el archivo .db no existe pero existe un .7z comprimido, extrae la base
   * de datos automáticamente usando 7-Zip. Configura la base de datos como solo
   * lectura y prepara todos los prepared statements necesarios para optimizar
   * las consultas.
   *
   ...
   *   // Ahora se pueden hacer búsquedas
   *   const results = database.search('archivo.zip');
   * } else {
   ...
   *
   * Busca tablas FTS en la base de datos y extrae información sobre el nombre de la tabla,
   * tipo (FTS5/FTS4), y columnas indexadas. Esta información se usa para optimizar las
   * búsquedas usando FTS en lugar de LIKE cuando está disponible.
   *
   * @private
   * @returns {FTSTableInfo|null} Objeto con información de la tabla FTS, o null si no hay FTS
   *
   * @example
   * const ftsInfo = this._detectFTS();
   * if (ftsInfo) {
   *   console.log(`Tabla FTS detectada: ${ftsInfo.name} (${ftsInfo.type})`);
   *   console.log(`Columnas indexadas: ${ftsInfo.columns.join(', ')}`);
   * } else {
   *   console.log('No hay tablas FTS, usando búsquedas LIKE');
   * }
   */
  _detectFTS() {
    try {
  ...
  _prepareStatements() {
    ...
    this.statements = {
      ...
      getNodeById: this.db.prepare('SELECT id, parent_id, title, type FROM nodes WHERE id = ?'),

      // Query optimizada con pluck() para obtener solo el título
      getNodeTitle: this.db.prepare('SELECT title FROM nodes WHERE id = ?').pluck(),

      getNodeWithUrl: this.db.prepare('SELECT url, title FROM nodes WHERE id = ?'),
  ...
  _searchWithLike(term, limit = 500, offset = 0) {
    ...
    log.debug(
      `Búsqueda LIKE mejorada "${term}": ${results.length} resultados (limit: ${limit}, offset: ${offset})`
    );
    return results;
  }
  ...
  search(searchTerm, options = {}) {
    ...
    const { limit = 500, offset = 0, usePrefix = true, usePhrase = false, useOR = false } = options;
    ...
    try {
      ...
      if (this.useFTS && this.statements.searchFTS) {
        ...
        log.debug(
          `Búsqueda FTS "${cleanSearchTerm}" -> "${ftsTerm}" (limit: ${limit}, offset: ${offset})`
        );
